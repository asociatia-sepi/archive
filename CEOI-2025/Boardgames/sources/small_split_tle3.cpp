...
void dfs2(std::vector<int> &q, int &cap, int x, int l, int r, int L, int R){
    if(cap == 0) return;
    if(x < l || r <= x){
        cap = 0;
        return;
...
void partition(int l, int r){
    ...
    for(int i = 0; i < (r-l) / 5; i++){
        ...
        std::vector<int> q;
        if(i%2 == 0){
            dfs2(q, cap, l + i/2, l, l + (r - l) / 10, l, r);
        } else {
            dfs2(q, cap, r - i/2 - 1, r - (r - l) / 10, r, l, r);
        }
        ...
        if(cap > 0){
            ...
            for(int j = 0; j < (int)q.size(); j++){
                if(j == 0 || q[j] != q[j-1]+1){
                    if(last != q[j]) partition(last, q[j]);
                    last = q[j];
                }
