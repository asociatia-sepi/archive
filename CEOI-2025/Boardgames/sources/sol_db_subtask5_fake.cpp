...
void calc_spt(int N)
{
    for (int i = 1; i <= N; ++i)
    {
        sptmin[0][i] = path_min(i, i + 1);
        sptmax[0][i] = path_max(i, i + 1);
    }
    for (int j = 1; j < L; ++j)
    {
        for (int i = 1; i + (1 << j) <= N + 1; ++i)
        {
            sptmin[j][i] = min(sptmin[j - 1][i], sptmin[j - 1][i + (1 << j - 1)]);
            sptmax[j][i] = max(sptmax[j - 1][i], sptmax[j - 1][i + (1 << j - 1)]);
        }
    }
}
...
std::vector<int> partition_players(int N, int M, std::vector<int> X, std::vector<int> Y)
{
    ...
    {
        int lgx = 31 - __builtin_clz(r - l + 1);
        return min(sptmin[lgx][l], sptmin[lgx][r - (1 << lgx) + 1]);
    };
    auto query_max = [](int l, int r)
    {
        int lgx = 31 - __builtin_clz(r - l + 1);
        return max(sptmax[lgx][l], sptmax[lgx][r - (1 << lgx) + 1]);
    };
    auto process = [&](int l, int r)
    {
        if (query(r - 1) - query(l - 1) >= 1)
        {
            return make_pair(0, maxN - 1);
        }
        return make_pair(query_min(l, r - 1), query_max(l, r - 1));
    };
    vector<int> ans;
    int l = 1;
    for (int i = 1; i < N;)
    {
        ...
        while (true)
        {
            pii q = process(p.xx, p.yy);
            if (q.xx == 0 || q.yy == maxN - 1)
            {
                ans.push_back(i - l + 1);
                add(i);
                l = i = i + 1;
